...
class Processor:
    ...
    def get_digit(self, cell, debug=False):
        ...
        # Ensure that contours exist (if it exists, it is not an empty cell but if the length is 0, then contour dne)
        if len(contours) == 0:
        ...
        if (percentFilled < 0.03):
            return None # did not find a valid digit
        ...
        if debug == True:
            ...
            cv.waitKey(0)
        return digit
    # For each cell we get either the number filled or no number at all (0)
    # We return a NumPy board that reflects the sudoku puzzle 
    # Must convert the board into a standard Python Matrix (like the one in sudoku.py)
    def extract_board(self):
        ...
        for y in range(0, 9):
            ...
            for x in range(0, 9):
                ...
                digit = self.get_digit(cell, True)
                
                if digit is not None:
                    digit = cv.resize(digit, (28, 28))

                    # Extract the text from the cell
                    text = pytesseract.image_to_string(self.image, config="--psm 6 -c tessedit_char_whitelist=0123456789")
    ...
    def solve_puzzle(self):
        ...
        # tolist from the NumPy module 
        matrix = self.board.tolist()
    ...
    def insert_values(self):
        ...
        for emtpty in self.empties:
            x, y = emtpty
            originX = self.stepX * x + (self.stepX // 2)
            originY = self.stepY * y + (self.stepY // 2)
            cv.putText(solved, str(mat[x][y]), (originX, originY), cv.FONT_HERSHEY_PLAIN, 1, (0, 100, 0), 1)
        return solved
